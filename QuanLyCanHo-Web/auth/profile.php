...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    // Xử lý xóa ảnh đại diện khi xác nhận
    if (($_POST['action'] ?? '') === 'delete_avatar') {
        if (!empty($user['Avatar'])) {
            ...
            $stmtDel->execute([$maNv]);
            $_SESSION['Avatar'] = null;
            setFlash('success', 'Đã xóa ảnh đại diện thành công!');
        }
...
<div id="deleteAvatarModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 2000; align-items: center; justify-content: center;">
    <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(15, 23, 42, 0.65); backdrop-filter: blur(4px);" onclick="closeDeleteAvatarModal()"></div>
    <div style="position: relative; background: #ffffff; padding: 2.25rem; border-radius: 16px; max-width: 420px; width: 90%; text-align: center; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.25); z-index: 2001; border: 1px solid #e2e8f0;">
        ...
        <div style="display: flex; gap: 0.85rem; justify-content: center;">
            <button type="button" class="btn btn-outline" onclick="closeDeleteAvatarModal()" style="min-width: 110px; height: 42px; font-weight: 600; border-radius: 8px;">
                Không
            </button>
...
function openDeleteAvatarModal() {
    const m = document.getElementById('deleteAvatarModal');
